...
const SignUp = (props) => {
  ...
  function getData() {
    if (password === confirmPassword) {
      firebase.db
        .collection("patauthentification")
        .doc(email)
        .get()
        .then((doc) => {
          if (doc.exists) {
  ...
  function getDocdata() {
    firebase.db
      .collection("docauthentification")
      .doc(email)
      .get()
      .then((doc) => {
        if (doc.exists) {
          ...
          alert(
            "You are not authorized to Login. Please contact the hospital for assistance"
          );
  ...
  function DocsubmitSignUp() {
    try {
      if (type === "doctor") {
        ...
        alert("Please SignUp using Doctor SignUp portal");
        props.history.replace("/choice");
      }
    } catch (error) {
      alert(error.message);
  ...
  function submitSignUp() {
    try {
      ...
      if (type === "patient") {
        ...
        alert("Please SignUp using Patient SignUp portal");
        props.history.replace("/choice");
      }
    } catch (error) {
      alert(error.message);
              ...
              <div className={"mt-6 pb-8"}>
                <div className={" mx-auto px-3"}>
                  ...
                  >
                    ...
                    <NavLink to={"/choice"}>Login</NavLink>
                  </div>
                </div>
              </div>
            </div>
